...
export function computeCDF (data) {
  let cdf = _.sortBy(data).map((value, idx) => {
    const count = idx + 1
  ...
  cdf = Object.entries(cdf.reduce((a, b) => {
    ...
    const risk = 100 * Number.parseFloat(v[0])
    const ratio = (100 * v[1]) / data.length
...
export function JSONToArray (strData, hasHeader) {
  ...
  function dumpObject (obj, prefix) {
    for (const prop in obj) {
      const currentPath = prefix + prop
      const currentVal = obj[prop]

      if (currentVal === null || isPrimitive(currentVal)) {
        if (headers.indexOf(currentPath) === -1) headers.push(currentPath)
        data[data.length - 1][currentPath] = currentVal
      } else {
        dumpObject(currentVal, currentPath + '.')
      }
    }
  }
